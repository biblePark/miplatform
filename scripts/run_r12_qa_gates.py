...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="run_r12_qa_gates",
        description="Execute the R12 QA gate test set.",
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="Run the full test suite (`unittest discover`) instead of the R12 gate subset.",
    )
    parser.add_argument(
        "--fail-fast",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[run_r12_qa_gates] {' '.join(command)}")
