...
2. `run_real_sample_e2e_regression.py`로 실샘플 회귀 실행
3. `out/real-sample-e2e-regression/regression-summary.json`에서 실패/위험 추세 확인
4. 실패 샘플은 `runs/<순번>-<xml-stem>/reports/` 하위 단계 리포트로 원인 분해
5. 필요 시 단일 `migrate-e2e` 또는 개별 명령(`parse`/`map-api`/`gen-ui`/`sync-preview`)로 재검증
