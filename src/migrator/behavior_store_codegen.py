...
def _transaction_action_name_union(transaction_actions: list[BehaviorActionSpec]) -> str:
    if not transaction_actions:
        return "never"
    return " | ".join(_to_js_string(spec.action_name) for spec in transaction_actions)
...
def _render_actions_module(
    *,
    ...
    transaction_actions = [spec for spec in actions if spec.source_kind == "transaction"]
    transaction_action_name_union = _transaction_action_name_union(transaction_actions)
    ...
    lines.extend(
        [
            "];",
            "",
            "export interface ScreenBehaviorTransactionContract {",
            "  actionName: string;",
            ...
            f"  {transaction_action_name_union};",
            "",
            "export const screenBehaviorTransactionContracts: ScreenBehaviorTransactionContract[] = [",
        ]
    )

    for spec in transaction_actions:
        lines.extend(
            [
                "  {",
                f"    actionName: {_to_js_string(spec.action_name)},",
                f"    transactionId: {json.dumps(spec.transaction_id, ensure_ascii=False)},",
                f"    method: {json.dumps((spec.method or '').strip().upper() or None, ensure_ascii=False)},",
                f"    endpoint: {json.dumps((spec.endpoint or '').strip() or None, ensure_ascii=False)},",
                f"    trigger: {_to_js_string(_action_anchor_trigger(spec))},",
                f"    source: {_source_literal(spec.source)},",
                "  },",
            ]
    ...
    lines.extend(
        [
            "];",
            "",
            "const screenBehaviorTransactionContractsByActionName: Record<",
            "  ScreenBehaviorTransactionActionName,",
            "  ScreenBehaviorTransactionContract",
            "> = {",
        ]
    )

    for spec in transaction_actions:
        lines.append(f"  {_to_js_string(spec.action_name)}: {{")
        lines.extend(
            [
                f"    actionName: {_to_js_string(spec.action_name)},",
                f"    transactionId: {json.dumps(spec.transaction_id, ensure_ascii=False)},",
    ...
    lines.extend(
        [
            "};",
            "",
            "export interface ScreenBehaviorTransactionRequestEnvelope {",
            "  actionName: ScreenBehaviorTransactionActionName;",
            "  contract: ScreenBehaviorTransactionContract;",
            "}",
            "",
            "export interface ScreenBehaviorTransactionResponseEnvelope {",
            "  ok: boolean;",
            "  status: number | null;",
            "  data: unknown;",
            "  error: unknown;",
            "}",
            "",
            "export interface ScreenBehaviorTransactionErrorEnvelope {",
            "  actionName: ScreenBehaviorTransactionActionName;",
            '  phase: "request" | "response";',
            "  message: string;",
            "  request: ScreenBehaviorTransactionRequestEnvelope;",
            "  response: ScreenBehaviorTransactionResponseEnvelope | null;",
            "  cause: unknown;",
            "}",
            "",
            "export interface ScreenBehaviorTransactionAdapterHooks {",
            "  onRequest?: (",
            "    request: ScreenBehaviorTransactionRequestEnvelope,",
            "  ) =>",
            "    | ScreenBehaviorTransactionResponseEnvelope",
            "    | Promise<ScreenBehaviorTransactionResponseEnvelope>",
            "    | void",
            "    | Promise<void>;",
            "  onResponse?: (",
            "    response: ScreenBehaviorTransactionResponseEnvelope,",
            "    request: ScreenBehaviorTransactionRequestEnvelope,",
            "  ) =>",
            "    | ScreenBehaviorTransactionResponseEnvelope",
            "    | Promise<ScreenBehaviorTransactionResponseEnvelope>",
            "    | void",
            "    | Promise<void>;",
            "  onError?: (error: ScreenBehaviorTransactionErrorEnvelope) => void | Promise<void>;",
            "}",
            "",
            "export interface CreateScreenBehaviorActionsOptions {",
            "  transactionAdapters?: Partial<",
            "    Record<ScreenBehaviorTransactionActionName, ScreenBehaviorTransactionAdapterHooks>",
            "  >;",
            "  defaultTransactionAdapter?: ScreenBehaviorTransactionAdapterHooks;",
            "}",
            "",
            "function toScreenBehaviorTransactionResponseEnvelope(",
            "  value: ScreenBehaviorTransactionResponseEnvelope | void,",
            "  request: ScreenBehaviorTransactionRequestEnvelope,",
            "): ScreenBehaviorTransactionResponseEnvelope {",
            "  if (value !== undefined) {",
            "    return value;",
            ...
            "    data: null,",
            "    error: {",
            '      code: "UNIMPLEMENTED_TRANSACTION_ADAPTER",',
            '      message: `Transaction adapter hook is not configured for ${request.actionName}.`,',
            "      transactionId: request.contract.transactionId,",
            "      endpoint: request.contract.endpoint,",
            "      method: request.contract.method,",
            "    },",
            "  };",
            "}",
            "",
            "async function runScreenBehaviorTransactionAction(",
            "  actionName: ScreenBehaviorTransactionActionName,",
            "  options?: CreateScreenBehaviorActionsOptions,",
            "): Promise<void> {",
            "  const contract = screenBehaviorTransactionContractsByActionName[actionName];",
            "  const requestEnvelope: ScreenBehaviorTransactionRequestEnvelope = {",
            "    actionName,",
            "    contract,",
            "  };",
            "  const adapter =",
            "    options?.transactionAdapters?.[actionName] ??",
            "    options?.defaultTransactionAdapter;",
            "",
            "  let responseEnvelope: ScreenBehaviorTransactionResponseEnvelope | null = null;",
            "  try {",
            "    const requestResult = await adapter?.onRequest?.(requestEnvelope);",
            "    responseEnvelope = toScreenBehaviorTransactionResponseEnvelope(",
            "      requestResult,",
            "      requestEnvelope,",
            "    );",
            "    const responseResult = await adapter?.onResponse?.(",
            "      responseEnvelope,",
            "      requestEnvelope,",
            "    );",
            ...
            "    }",
            "  } catch (cause) {",
            "    const requestErrorEnvelope: ScreenBehaviorTransactionErrorEnvelope = {",
            "      actionName,",
            '      phase: "request",',
            '      message: `Transaction request hook failed for ${actionName}.`,',
            "      request: requestEnvelope,",
            "      response: responseEnvelope,",
            "      cause,",
            "    };",
            "    await adapter?.onError?.(requestErrorEnvelope);",
            "    throw requestErrorEnvelope;",
            "  }",
            "",
            ...
            "    const responseErrorEnvelope: ScreenBehaviorTransactionErrorEnvelope = {",
            "      actionName,",
            '      phase: "response",',
            '      message: `Transaction response indicated failure for ${actionName}.`,',
            "      request: requestEnvelope,",
            "      response: responseEnvelope,",
            "      cause: responseEnvelope.error,",
            "    };",
            "    await adapter?.onError?.(responseErrorEnvelope);",
            "    throw responseErrorEnvelope;",
            "  }",
            "}",
            "",
    ...
    lines.extend(
        [
            "}",
            "",
            "export function createScreenBehaviorActions(",
            "  options?: CreateScreenBehaviorActionsOptions,",
            "): ScreenBehaviorActions {",
        ]
    )
    lines.append("  return {")
    for spec in actions:
        if spec.source_kind == "transaction":
            lines.extend(
                [
                    ...
                    f"      await runScreenBehaviorTransactionAction({_to_js_string(spec.action_name)}, options);",
                    "    },",
                ]
            )
        else:
