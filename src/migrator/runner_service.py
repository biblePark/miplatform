...
CooperativeCancelHook = Callable[[str, "OrchestratorJobRequest"], bool]
BatchScheduledHook = Callable[[str, tuple[str, ...]], None]
...
class RunnerService:
    def __init__(
        ...
        *,
        ...
        self._pipeline_runner = pipeline_runner or run_migrate_e2e
        self._cooperative_cancel_hook = cooperative_cancel_hook
        self._batch_scheduled_hook = batch_scheduled_hook
    ...
    def set_cooperative_cancel_hook(self, hook: CooperativeCancelHook | None) -> None:
        self._cooperative_cancel_hook = hook

    def set_batch_scheduled_hook(self, hook: BatchScheduledHook | None) -> None:
        self._batch_scheduled_hook = hook
        ...
        *,
        ...
        if isinstance(payloads, (str, bytes)) or not isinstance(payloads, Sequence):
            raise OrchestratorApiError(
                400,
                code="invalid_payload",
                message="`payloads` must be an array of job payload objects.",
                details={"field": "payloads"},
            )
        ...
        if not normalized_payloads:
            raise OrchestratorApiError(
                ...
                message="`payloads` must contain at least one job payload object.",
                details={"field": "payloads", "minimum": 1, "actual": 0},
            )

        resolved_batch_id = self._resolve_batch_id(batch_id)
        batch_size = len(normalized_payloads)
        jobs: list[dict[str, Any]] = []
        for index, payload in enumerate(normalized_payloads, start=1):
            created = self.create_job(payload)
            job_id = created.get("id")
            if isinstance(job_id, str):
                self._annotate_batch_context(
                    job_id,
        ...
        if self._batch_scheduled_hook is not None:
            self._batch_scheduled_hook(
                resolved_batch_id,
                tuple(job["id"] for job in jobs if isinstance(job.get("id"), str)),
            )
    ...
    def _resolve_batch_id(self, batch_id: str | None) -> str:
        if batch_id is None:
            return uuid4().hex
        if not isinstance(batch_id, str) or not batch_id.strip():
            raise OrchestratorApiError(
                400,
                code="validation_error",
                message="`batch_id` must be a non-empty string when provided.",
                details={"field": "batch_id"},
            )
        return batch_id.strip()

    def _annotate_batch_context(
        self,
        job_id: str,
        *,
        batch_id: str,
        ...
        with self._lock:
            ...
            self._append_log_locked(
                job,
                level="info",
                event="batch_scheduled",
                message="Job scheduled via batch request.",
                details={
                    "batch_id": batch_id,
    ...
    def _promote_cooperative_cancel_request(
        self,
        job_id: str,
        request: OrchestratorJobRequest,
    ) -> None:
        hook = self._cooperative_cancel_hook
        if hook is None:
            return
        try:
            requested = bool(hook(job_id, request))
        ...
        with self._lock:
            ...
            current.cancel_requested_at_utc = now
            self._append_log_locked(
                current,
                level="info",
                event="cancel_requested",
                message="Cancel request accepted by cooperative hook.",
            )
            self._persist_jobs_locked()
    ...
    def _execute_job(self, job_id: str) -> None:
        ...
        try:
            self._promote_cooperative_cancel_request(job_id, request)
            ...
            exit_code = self._pipeline_runner(request.to_cli_namespace())
            summary_path = (
